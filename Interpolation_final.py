...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.label_ovelaping = QtWidgets.QLabel(self.layoutWidget1)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.Preferred, QtWidgets.QSizePolicy.Preferred)
        sizePolicy.setHorizontalStretch(0)
        sizePolicy.setVerticalStretch(5)
        sizePolicy.setHeightForWidth(self.label_ovelaping.sizePolicy().hasHeightForWidth())
        self.label_ovelaping.setSizePolicy(sizePolicy)
        font = QtGui.QFont()
        font.setFamily("Yu Gothic")
        font.setBold(True)
        font.setWeight(75)
        self.label_ovelaping.setFont(font)
        self.label_ovelaping.setObjectName("label_ovelaping")
        self.verticalLayout_for_equ.addWidget(self.label_ovelaping)
        self.lineEdit_overlap = QtWidgets.QLineEdit(self.layoutWidget1)
        ...
        self.verticalLayout_for_equ.addWidget(self.lineEdit_overlap)
        ...
        self.figure_canvas_map = FigureCanvas(self.map_figure)
        self.horizontalLayout_mapgraph.addWidget(self.figure_canvas_map)
       # axes = self.map_figure.gca()
       # axes.cla()
        ...
        self.generate_error_map_button.clicked.connect(lambda:self.toggle_button)
        self.movement = 0
        self.time_col = []
        self.amp_col = []
    ...
    def currentIndex(self, x, y):
        sum = x + y
        dic_labels = {bool(sum == 3): "overlapping between chunks",
                      bool(sum == 4): "The order of the fitting polynomial", bool(sum == 5): "Number of chunks"}
        return self.label_ovelaping.setText(dic_labels.get(True))
    ...
    def get_fitting_order(self):
        if self.fitting_order_lineEdit.text() != "":
            self.fitting_order = self.fitting_order_lineEdit.text()
        return int(self.fitting_order)
    ...
    def Generate_error_map(self):
        ...
        self.overlap_percentage = int(self.lineEdit_overlap.text())
        arr = []
        for i in range(rows):
            col = []
            for j in range(cols):
                dic = {bool(x == 1 and y == 2): self.calculate_error_map(j + 1, i + 1, self.overlap_percentage / 100),
                       ...
                       bool(x == 3 and y == 1): self.calculate_error_map(i + 1, self.overlap_percentage,overlap_list[j] / 100),
                       bool(x == 1 and y == 3): self.calculate_error_map(j + 1, self.overlap_percentage,overlap_list[i] / 100),
                       bool(x == 3 and y == 2): self.calculate_error_map(self.overlap_percentage, i + 1, overlap_list[j] / 100),
                       bool(x == 2 and y == 3): self.calculate_error_map(self.overlap_percentage, j + 1, overlap_list[i] / 100)}
        ...
        self.figure_canvas_map.draw()
        self.figure_canvas_map.flush_events()
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label_extra.setText(_translate("MainWindow", "Percentage Slider for extrapolation"))
        ...
        self.label_ovelaping.setText(_translate("MainWindow", "overlapping percentage"))
